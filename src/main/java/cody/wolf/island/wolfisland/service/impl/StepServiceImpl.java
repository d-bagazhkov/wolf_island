...
public class StepServiceImpl implements StepService {
    ...
    public List<Entity> handle() {
        if (entities == null) return reset();
        if (entities.stream().noneMatch(e -> hasText(e.getEntityName()))) {
            return reset();
        }
        forI(islandConfig.getCountHorizontalCeil(), x -> {
            forI(islandConfig.getCountVerticalCeil(), y -> {
                Entity entity = get(x, y, entities);
                if (!hasText(entity.getEntityName())) return;
                randomMove(entity, entities);
            });
        });
    ...
    public List<Entity> reset() {
        entities = IntStream.range(0, islandConfig.getCountHorizontalCeil()).boxed()
